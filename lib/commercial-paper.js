...
function purchasePaper(purchaseOrder) {
  	...
  	return query(ownershipsQuery, { paper: 'resource:' + purchaseOrder.paper.getFullyQualifiedIdentifier() })
  			.then(function(ownerships) {
                ...
                ownerships.forEach(function(ownership) {
                  if (ownership.owner.getFullyQualifiedName() === getCurrentParticipant().getFullyQualifiedName()) {
  	...
  	return getAssetRegistry('fabric.ibm.commercialpaper.PaperOwnership')
  		.then(function(assRegistry) {
			return getParticipantRegistry(purchaseOrder.buyer.getFullyQualifiedType());
        })
...
function validatePurchaseOrder(purchaseOrder) {
  	...
  	var cusipErr = validateCUSIPMatchesPaper(purchaseOrder.CUSIP, purchaseOrder.paper);
	if (cusipErr.error) {
		errors.push(cusipErr.msg);
    }
  
  	var ownerErr = validateParticipantIsCurrentParticipant(purchaseOrder.owner);
  	if (ownerErr.error) {
...
function validateCUSIPMatchesPaper(cusip, paper) {
  	if (!(paper.CUSIP === cusip)) {
      	return { "error": true, "msg": "Given CUSIP does not match paper's CUSIP" };
    }
...
function verifyBalance(buyer, amount, paper) {
